...
class RequestProcessor(RequestGenerator):
    ...
    def process(self, time):
        if self._queue.current_queue_size > 0:
            ...
            if rn.random_sample() < self._return_probability:
...
class Model:
    def __init__(self, s1, m2, s2, n1, n2, ret_prob):
        self._generators = [RequestGenerator(ReleyGenerator(s1[i]), i + 1) for i in range(n1)]
        self._processors = [RequestProcessor(GaussGenerator(m2[0], s2[0]),
                                             GaussGenerator(m2[1], s2[1]), ret_prob) for i in range(n2)]
